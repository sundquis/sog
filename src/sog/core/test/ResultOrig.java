 ...
 *
 */
@TestOrig.Skip( "How to test abstract?" )
public abstract class ResultOrig {
	...
	private final Map<String, ResultOrig> children;
	...
	protected ResultOrig( String label ) {
		this.label = Assert.nonEmpty( label );
		this.children = new TreeMap<String, ResultOrig>();
	...
	private void setStats() {
		synchronized ( this.statMutex ) {
			if ( this.time < 0L ) {
				...
				int t = 0;
				long time = 0L;
				for ( ResultOrig r : this.children.values() ) {
					p += r.getPassCount();
					f += r.getFailCount();
					t += r.getTotalCount();
					time += r.getTime();
	...
	public ResultOrig addChild( ResultOrig child ) {
		ResultOrig result = Assert.nonNull( child );
		
		synchronized ( this.children ) {
			ResultOrig current = this.children.get( result.getLabel() );
			if ( current == null ) {
